...
static void mcu_adc_start_cnv(adc_ctx_t *ctx) {
    if (!ctx || !ctx->hw_cfg.hadc) return;

    ADC_HandleTypeDef *hadc = (ADC_HandleTypeDef *)ctx->hw_cfg.hadc;
    ADC_ChannelConfTypeDef sConfig = {
        .Channel      = ctx->hw_cfg.channel,
        .Rank         = ADC_REGULAR_RANK_1,
        .SamplingTime = LL_ADC_SAMPLINGTIME_12CYCLES_5
    };

    HAL_ADC_ConfigChannel(hadc, &sConfig);
    HAL_ADC_Start(hadc); // Chỉ phát lệnh Start ở đây
}
...
static uint16_t mcu_adc_read_raw(adc_ctx_t *ctx) {
	...
	// Nếu trước đó chưa gọi start, có thể poll luôn ở đây
...
void adc_create_obj(adc_objt_t *p_adc_obj) {
    if (!p_adc_obj) return;
    p_adc_obj->method->start_cnv       = mcu_adc_start_cnv;
